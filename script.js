...
async function useFallbackAPI(userMessage) {
    try {
        ...
        const fallbackRequestOptions = {
            ...
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('fallback_api_key') || DEFAULT_API_KEY}`,
...
async function testAPIConnection() {
    ...
    try {
        ...
        if (!response.ok) {
            console.error('API connection test failed, using OpenRouter fallback');
        } else {
